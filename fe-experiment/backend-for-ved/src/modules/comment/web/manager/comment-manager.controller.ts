...
import { CommentMarkAsReadByManagerDto, CommentUpdateByManagerDto } from 'modules/comment/dto/comment.update.dto';
import { IPaginateHasNextResult } from 'lib/interfaces/paginate.interface';
import {
  CommentAdminQueryDto,
  CommentManagerPaginateDto,
  EntitiesByUnreadCommentsQueryDto,
} from 'modules/comment/dto/comment.query.dto';
import { CommentCreateByManagerDto } from 'modules/comment/dto/comment.create.dto';
...
@Controller('manager/comment')
export class CommentManagerController {
  constructor(@Inject(COMMENT_SERVICE) private readonly service: ICommentService) {}

  @Get()
  @ManagerMethod({ hasNextPaginate: CommentDto })
  async findWithPaginate(@Query() dto: CommentManagerPaginateDto): Promise<IPaginateHasNextResult<IComment>> {
    const { paginate, model } = queryPaginateParser(dto, CommentAdminQueryDto);
  ...
  @Post()
  @ManagerMethod({ response: { status: 200, type: CommentDto } })
  async createByAdmin(@Req() req: Request, @Body() dto: CommentCreateByManagerDto): Promise<IComment> {
    const comment = await this.service.create({ ...dto, account: req.account._id });
  ...
  async patchById(
    @Req() req: Request,
    @Param('_id') id: string,
    @Body() updateDto: CommentUpdateByManagerDto,
  ): Promise<IComment> {
  ...
  async markAsRead(@Req() req: Request, @Body() updateDto: CommentMarkAsReadByManagerDto): Promise<void> {
    await this.service.markAsRead({ ...updateDto, account: req.account._id });
  }
  ...
  async findEntitiesIdsWithUnreadComments(@Req() req: Request, @Query() dto: EntitiesByUnreadCommentsQueryDto) {
    return this.service.findEntitiesIdsByUnreadComments({
      ...dto,
      account: req.account,
      kind: [CommentKind.EXTERNAL, CommentKind.INTERNAL],
    });
  }
