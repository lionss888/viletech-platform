...
describe("create review OCR copy", () => {
  it("mentions recognition, draft/submit split, and Vedi assistant", () => {
    ...
    expect(CREATE_REVIEW_OCR_BANNER).toMatch(/вручную/i);
    expect(CREATE_REVIEW_OCR_BANNER).not.toMatch(/подставятся сами/i);
    ...
    expect(CREATE_REVIEW_OCR_PENDING).not.toMatch(/подставятся сами/i);
  ...
  it("covers unavailable, degraded, and auth_lost without promising autofill", () => {
    ...
    expect(CREATE_REVIEW_OCR_DEGRADED).toMatch(/ограничен/i);
    expect(CREATE_REVIEW_OCR_DEGRADED).toMatch(/автоподстанов/i);
    expect(CREATE_REVIEW_OCR_DEGRADED).not.toMatch(/подставятся сами/i);
