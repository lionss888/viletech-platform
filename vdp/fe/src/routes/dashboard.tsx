...
function RootDashboard() {
  ...
  const cards = [
    ...
    { label: "Заявок в работе", value: String(stats.active), to: "/forms", search: {} },
    { label: "Зависшие заявки", value: String(stats.stuck.length), to: "/forms", search: { stuck: true } },
  ] as const;
  ...
  return (
    <AppShell title="Рабочий стол · Суперадмин" subtitle={ROLE_FOCUS.root}>
      ...
      <section className="panel mt-4 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <Link to="/forms" search={{ stuck: true }} className="text-xs font-semibold text-muted-foreground hover:text-foreground">
            Реестр →
          </Link>
        </div>
        <ul className="mt-3 divide-y divide-border">
...
function RoleDashboard() {
  ...
  return (
    <AppShell title={`Рабочий стол · ${roleTitle(role)}`} subtitle={ROLE_FOCUS[role]}>
      {compliance && (
        <div className="mb-4 grid gap-3 sm:grid-cols-3">
          <Link to="/forms" search={{ mine: true }} className="panel block p-4 transition-colors hover:border-accent">
            <p className="label-caps">Входящие заявки</p>
            <p className="mt-1 font-mono text-2xl font-semibold">{todo.length}</p>
          </Link>
          <Link to="/organizations" className="panel block p-4 transition-colors hover:border-accent">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {[
          ...
          <Link key={card.label} to="/forms" search={card.search} className="panel block p-4 transition-colors hover:border-accent">
            <p className="label-caps">{card.label}</p>
            <p className="mt-1 font-mono text-2xl font-semibold">{card.value}</p>
          </Link>
        ))}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,3fr)_minmax(0,1fr)]">
        ...
        <section className="panel p-4">
          <div className="flex items-center justify-between gap-2">
            ...
            <Link to="/forms" search={{ mine: true }} className="text-xs font-semibold text-muted-foreground hover:text-foreground">
              Все заявки →
            </Link>
          </div>
            ...
            <ul className="mt-3 divide-y divide-border">
              {todo.slice(0, 7).map((form) => (
                <li key={form.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 py-2.5">
                  <Link to="/forms/$id" params={{ id: form.id }} className="shrink-0 font-mono text-xs font-semibold hover:underline">
                    {form.number}
                  </Link>
