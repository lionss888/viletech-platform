...
test.describe("Bank channel badge", () => {
  ...
  test("root creates bank form via /testing and sees badge on card", async ({ page, loginAs }) => {
    ...
    await page.getByRole("button", { name: "Создать bank-заявку" }).click();
    const result = page.locator("text=/Создана заявка .+ channel=bank/");
    ...
    await expect(page.getByText(/^corr:/)).toBeVisible();
  });
});
