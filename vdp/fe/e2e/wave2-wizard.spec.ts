...
/** No-docs path: single terms screen with draft-only CTA. */
async function openNoDocsDraftStep(page: Page): Promise<void> {
  ...
  await expect(page.getByTestId("wizard-no-documents-alert")).toBeVisible();
  await expect(page.getByTestId("wizard-extraction-dialog-trigger")).toHaveCount(0);
  await expect(page.getByTestId("wizard-send-manager")).toHaveCount(0);
  await expect(page.getByLabel(/Номер контракта/i)).toHaveCount(0);
}

async function fillInvoicePathToParties(
  page: Page,
...
test.describe("Wave2 wizard / OCR / submit", () => {
  ...
  test("docs first: invoice-only reaches review without contract", async ({ page, loginAs }) => {
    ...
    await expect(page.getByTestId("wizard-docs-step")).toBeVisible();
    await expect(page.getByTestId("wizard-invoice-ocr-help")).toBeVisible();
  ...
  test("no-docs: single step saves draft only", async ({ page, loginAs }) => {
    await loginAs("user");
    await page.goto("/forms/new");
    await openNoDocsDraftStep(page);
  ...
  test("preview: send to manager reaches waiting verification", async ({ page, loginAs }) => {
    await loginAs("user");
    await page.goto("/forms/new");
    await fillInvoicePathToParties(page, "postPayment");
  ...
  test("payment condition persists after create via API path", async ({ page, loginAs }) => {
    ...
    await loginAs("user");
    await page.goto("/forms/new");
    await fillInvoicePathToParties(page, "advance");
    await finishTermsAndReview(page);
    await saveWizardDraft(page);
    await expect(page.getByTestId("form-params")).toBeVisible({ timeout: 30_000 });
