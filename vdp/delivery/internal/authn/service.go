...
func New(secret string) *Service {
	if secret == "" {
		secret = "delivery-dev-secret"
	}
	svc := &Service{
		secret:     []byte(secret),
		localUsers: parseUsers(os.Getenv("DELIVERY_LOCAL_USERS")),
		githubMap:  parseRoleMap(os.Getenv("DELIVERY_GITHUB_ROLE_MAP")),
		gitlabMap:  parseRoleMap(os.Getenv("DELIVERY_GITLAB_ROLE_MAP")),
	}
