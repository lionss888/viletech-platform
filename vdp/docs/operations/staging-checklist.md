...
Alpha edge. Public https://alpha.vedy.io/api/v1/health returns 200 and seed login returns token. Reconfirmed 2026-09-15 Milestone 1: public health 200, service vdp-core status ok. The host is already serving the stack behind Caddy; bootstrap-host.sh is the documented one-time procedure and cannot be re-run from this workstation without a working deploy SSH key. Hub docs and mail listen on VM loopback only by design. Full staging-smoke against alpha requires SSH on-host curl to 127.0.0.1:8081 and 8090 and 8091. Deploy SSH key on this workstation was denied publickey on 2026-09-15; ops must refresh DEPLOY_SSH_KEY before remote smoke and rollback on alpha.
