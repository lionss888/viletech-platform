...
Вопрос второй. GitHub PAT с правами repo, workflow и write:packages, использованный для отладки CI/CD, отозван; новый токен создан только у заказчика или в его secret store, не у подрядчика. Статус не выполнено. Волна 0: любой PAT из переписки отозвать сразу на github.com/settings/tokens, не дожидаясь handover. Этот чеклист по-прежнему не закрывает gate передачи.
