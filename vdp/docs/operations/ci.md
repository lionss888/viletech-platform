...
GitHub Actions — канон: merge в main, теги vdp-v, GHCR, deploy. GitLab (группа [vdp888](https://gitlab.com/vdp888)) — вторичный форж: зеркало, параллельные MR, тот же тестовый контур. Настройка зеркала: [gitlab-setup.md](gitlab-setup.md). Памятка кнопки обновления: [how-to-update.md](how-to-update.md).
...
Далее vdp-lovable-sync.yml: fetch GitHub `lionss888/vdp` (Lovable), открыть PR в `vdp/fe`, не merge в main. На GitLab тот же UI-репо лежит как [vdp888/vdp](https://gitlab.com/vdp888/vdp) — отдельно от зеркала монорепо [vdp888/viletech-platform](https://gitlab.com/vdp888/viletech-platform); см. [gitlab-setup.md](gitlab-setup.md).
