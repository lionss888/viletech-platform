...
func TestIngestAndScore(t *testing.T) {
	t.Parallel()
	st := memory.New()
	_ = st.SetConsent(context.Background(), domain.Consent{AccountID: "a1", Enabled: true})
...
func TestTelegramSkippedWithoutConsent(t *testing.T) {
	t.Parallel()
	st := memory.New()
	...
	status, err := svc.Ingest(context.Background(), managerops.Event{
		IdempotencyKey: "k2", AccountID: "a1", Source: managerops.SourceTelegram,
		Kind: managerops.KindNudgeAck, OccurredAt: time.Now().UTC(),
	})
	if err != nil || status != "skipped_consent" {
		t.Fatalf("status=%s err=%v", status, err)
	}
}
