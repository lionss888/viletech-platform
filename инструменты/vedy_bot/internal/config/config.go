...
// Load reads env and optional INTAKE_HOME/env or ~/.vedy_bot/env (fallback ~/.vdp-intake/env).
func Load() (Config, error) {
	home := strings.TrimSpace(os.Getenv("INTAKE_HOME"))
	if home == "" {
		// Try ~/.vedy_bot first, fall back to legacy ~/.vdp-intake
		primary := filepath.Join(os.Getenv("HOME"), ".vedy_bot")
		legacy := filepath.Join(os.Getenv("HOME"), ".vdp-intake")
		if _, err := os.Stat(filepath.Join(primary, "env")); err == nil {
			home = primary
		} else if _, err := os.Stat(filepath.Join(legacy, "env")); err == nil {
			home = legacy
		} else {
			home = primary // default to new path
		}
	}
