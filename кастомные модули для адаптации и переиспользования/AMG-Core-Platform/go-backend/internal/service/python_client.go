...
func (c *PythonAnalyticsClient) ProcessChat(ctx context.Context, req *PythonChatRequest) (*PythonChatResponse, error) {
	url := fmt.Sprintf("%s/v1/ai/chat", c.baseURL)
...
// PythonModel представляет модель из Python сервиса
...
// PythonModelsResponse представляет ответ со списком моделей
type PythonModelsResponse struct {
	Models []PythonModel `json:"models"`
}

// GetModels получает список моделей из Python сервиса
func (c *PythonAnalyticsClient) GetModels(ctx context.Context) ([]PythonModel, error) {
	url := fmt.Sprintf("%s/v1/models", c.baseURL)
	
	httpReq, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return nil, errors.Wrap(err, errors.ErrCodeInternal, "Failed to create models request")
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, errors.NewWithDetails(
			errors.ErrCodePythonService,
			"Python models service returned error",
			fmt.Sprintf("Status: %d, Body: %s", resp.StatusCode, string(body)),
		)
	}

	var response PythonModelsResponse
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		return nil, errors.Wrap(err, errors.ErrCodePythonService, "Failed to decode models response")
	}
