...
import SequenceArchive from './sequence_archive';
...
export function SequenceFactory(connection) {
  ...
  class Sequence {
    ...
    constructor(schema: mongoose.Schema, opts) {
      const defaults = {
        id: null,
        inc_field: '_id',
        start_seq: 1,
    ...
    _resetCounter(id, reference, callback) {
      ...
      this._counterModel
        .updateMany(condition, { $set: { seq } }, null, cb)
        .exec()
        .then((result) => {
