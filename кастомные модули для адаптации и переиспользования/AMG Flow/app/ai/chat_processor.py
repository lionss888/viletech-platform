...
class AIChatProcessor:
    ...
    async def process_chat(self, request: ChatRequest) -> ChatResponse:
        """Обрабатывает чат запрос через AI."""
        try:
            ...
            # Отправляем в Ollama
            if request.stream:
                # Для стримминга возвращаем генератор
                return await self._process_streaming(request, messages)
            else:
                # Обычный запрос
    ...
    async def _process_regular(self, request: ChatRequest, messages: List[Dict[str, str]]) -> ChatResponse:
        ...
        if request.use_rag:
            await self._save_to_rag(request, response)
